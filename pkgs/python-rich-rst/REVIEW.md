...
| 2026-07-03 | 2.0.2-1 | (this PR) | upstream tag `v2.0.2` | safe-to-bump (Issue #332)。 bullet list レンダリング崩れの patch 修正のみ (`_render_bullet_list`)。 CLI に `--admonition-style` 追加 (console_script 未配布のため無関係)。 deps / build 無変化 (pyproject.toml dependencies 不変)。 sha256 独立検証済み (GitHub Release 実測 + asset digest 一致) |
| 2026-07-06 | 2.1.0-1 | (this PR) | upstream tag `v2.1.0` (commit `f8b0bf2c`、 GPG verified、 author Wasi Master) | safe-to-bump (Issue #349)。 中間 `v2.0.3` (sdist に `tests/conftest.py` 同梱の MANIFEST fix) の diff も compare に含まれ調査済み。 CLI に `-S`/`-d` フラグ追加・エラーメッセージ改善 (console_script 未配布のため無関係)。 deps / build 無変化 (pyproject.toml `dependencies` = `rich>=12.0.0` `pygments>=2.0.0` 不変、 build-system `setuptools>=64` 不変)。 sha256 独立検証済み (GitHub Release 実測 `134f8d18…` + asset digest 一致、 tarball 内 pyproject.toml も直接確認) |
