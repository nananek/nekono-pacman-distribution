...
| 2026-08-06 | 3.4.8-1 | (this PR) | `v3.4.8` (`a815cd3682ce4b01f28e93f4bc59f3d6d5db3d00`) | safe-to-bump (Issue #493)。 patch メンテナンスリリース。 実装 `.py` に差分なし (version 文字列のみ)。 runtime deps (`starlette`, `anyio`) 不変、変化は dev dep (`cryptography`, `pyasn1`) の dependabot bump (uv.lock のみ、PKGBUILD は uv 不使用)。 sha256 独立検証済み (GitHub archive 実測)。 |
