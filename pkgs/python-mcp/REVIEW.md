...
| 2026-05-26 | 1.27.1-4 | `ed05192` | — (pkgrel bump のみ) | `pkgrel` +1 (deps changed): python-starlette 1.0.0-1 → 1.1.0-1 |
| 2026-05-29 | 1.27.1-5 | (this PR) | — (pkgrel bump のみ) | `pkgrel` +1 (deps changed): python-pyjwt 2.12.1-1 → 2.13.0-1 |
...
| 2026-06-17 | 1.27.2-4 | (this PR) | — (pkgrel bump のみ) | `pkgrel` +1 (cascade: [nekono] dep rebuilt): python-sse-starlette 3.4.4-4 rebuild に追随 (build-all の cascade-warn 由来) |
...
| 2026-07-01 | 1.28.1-1 | (this PR) | `777b8d06710c140e3606b0d4598e2aa48546c266` | safe-to-bump: v1.27.2 → v1.28.1 (v1.x maintenance release、 Issue #321)。 **`1834.patch` を削除**: 旧 PR #1834 "Support Python 3.14" は upstream #2769 として `v1.27.2..v1.28.1` range に正式 merge 済み (v1.28.1 pyproject.toml に pydantic/starlette/pywin32 の `python_version >= '3.14'` conditional + `Programming Language :: Python :: 3.14` classifier を確認) → source[]/sha256sums/prepare() の patch 適用行を除去。 build()/package() 無変化、 depends 追加削除なし (pydantic/starlette は 3.14 marker 分岐が増えただけで単一 Arch Python では影響なし)。 tag→commit `777b8d06` / release author `maxisbey` を GitHub API で独立確認。 git source sha256 `a8d51d4314ab1c9a05e5643d7d0dea4bbfd43ea486183fde901ef083a0b23f99` は build host `makepkg --verifysource` で要確認。 `.deps.lock` の git 2.54.0-1 → 2.55.0-1 を fold (closed bot PR #318 分)。 ※ follow-up: `depends` の `python-opentelemetry-api` は v1.27.2/v1.28.1 の upstream pyproject.toml に見当たらず (Issue #321 指摘)、 次回 bump 時に「意図的差分」§の hard dep 記述が現行 upstream と整合するか要再確認 (今回は scope 外で維持) |
| 2026-07-03 | 1.28.1-2 | (this PR) | — (pkgrel bump のみ) | `pkgrel` +1 (cascade: [nekono] dep rebuilt): python-sse-starlette 3.4.5-1 rebuild に追随 (build-all の cascade-warn 由来、 build 前 fold で 1 round 化) |
| 2026-08-06 | 1.28.1-3 | (this PR) | — (pkgrel bump のみ) | `pkgrel` +1 (cascade: [nekono] dep rebuilt): python-sse-starlette 3.4.6-1 → 3.4.8-1 rebuild に追随 (build-all の cascade-warn 由来) |
