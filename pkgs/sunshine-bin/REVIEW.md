...
| 2026-09-16 | 2026.914.233613-1 | (this PR) | **high-severity security fix (Linux, GHSA-fp6g-27w5-489j)** — CAP_SYS_ADMIN/CAP_SYS_NICE 付与済みの sunshine プロセスが Qt/GTK/GIO/GdkPixbuf のモジュールローダー環境変数を特権実行前に継承していたため、launch 環境を制御できるローカル低権限ユーザが同 capability で任意ネイティブコードを load できる問題 (v0.19.0〜v2026.906.222525 が影響)。修正は特権 Linux 実行開始時に該当環境変数を除去するのみで CAP_SYS_ADMIN/CAP_SYS_NICE (KMS capture) は維持。tag `v2026.914.233613` target commit `63d35f702ee9e362e43263742981836ec0710384`、release author `LizardByte-bot` (公式 org)。asset を再 download し b2sum 独立再計算 `104008bd…` (PKGBUILD 値と一致)。`.PKGINFO` の depends 行、`ldd usr/bin/sunshine` の link 先、同梱 `.INSTALL` の内容 (setcap / udev trigger) を旧 release と diff、いずれも無変更 (FFmpeg 9 化は static link のため system depends 影響なし)。AUR 側はまだ本 version に追従していないが、独立検証で depends/build に変化が無いことを確認した上で先行 bump。`.deps.lock` 変更なし |
