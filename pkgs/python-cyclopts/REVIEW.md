...
**review 済み、 approve** (最新: 2026-08-30 / 4.23.3)
...
| 2026-08-30 | 4.23.3-1 | (this PR) | upstream tag `v4.23.3` | safe-to-bump (Issue #571)。`compare v4.23.2...v4.23.3` で変更ファイルは `cyclopts/config/_common.py` / `cyclopts/docs/base.py` / `cyclopts/help/*.py` / `cyclopts/utils.py` のみ、内容は deprecated docstring の help 描画修正・config JSON decode error のメッセージ整形・flattened subapp のドキュメント生成修正等の bugfix のみ。pyproject.toml / deps / build 無変化。sha256 独立検証済み (curl 実測 + PyPI JSON API 一致)。Closes #560, #571。 |
