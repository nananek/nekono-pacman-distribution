...
| 2026-09-16 | 5.5.2-1 | (this PR) | upstream tag `5.5.2` | safe-to-bump (Issue #634)。`compare/5.5.1...5.5.2` は `ap2_buffered_audio_processor.c` の1ファイルのみ (+82/-78)、release note曰く「OS 27 (macOS/iOS 27) 系で buffered audio stream に未知のデータブロックが来た際のハンドリング改善、先頭ブロックが未知だと再生開始しない不具合の修正」。`Makefile.am`/`configure.ac` に変更なし、depends/makedepends/configure_args 変更不要。tag target commitish は `master` (release author `mikebrady` 本人)。GitHub archive を再 download し sha256 を独立実測 `abcdb596…` へ更新、sysusers/patch (sha[1]/[2]) 据え置き。`remove_useradd.patch` は 5.5.2 でも dry-run 適用確認済み (Hunk #1 offset +21 で成功、5.5.1 と同一)。Closes #634。 |
